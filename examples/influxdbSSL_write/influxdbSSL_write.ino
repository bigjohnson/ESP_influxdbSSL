...
const char *INFLUXDB_HOST = "influxdb.panu.it";
const uint16_t INFLUXDB_PORT = 443;

const char *DATABASE = "dbname";
const char *DB_USER = "dbuser";
const char *DB_PASSWORD = "dbpassword";
// This is my ssl cert fingerprint, you must insert your's server fingerptint, see:
...
InfluxdbSSL influxdbSSL(INFLUXDB_HOST, INFLUXDB_PORT, FINGERPRINT);
...
void setup() {
        ...
        while (influxdbSSL.opendb(DATABASE)!=DB_SUCCESS) {
...
void loop() {
        ...
        Serial.println(influxdbSSL.write(row) == DB_SUCCESS ? "Object write success"
                       : "Writing failed");
        ...
        if (influxdbSSL.query(influxQL) != DB_SUCCESS)
